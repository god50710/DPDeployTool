...
class DeployTool(object):
    ...
    def add_cronjob(cls, data_site, build_path):
        ...
        if data_site == "beta" and not geoip_sync_cronjob:
            cls.run_command(
                "echo '0 * * * * aws s3 sync s3://spn-data-us-west-2/dataset/GeoIP/ s3://yamato-dp-aws-services-files-production-us-west-2/geoip/ >> /home/hadoop/cron.log' >> %s " %
                cronjob_file)
    ...
    def command_parser(cls):
        ...
        if len(sys.argv) == 1:
            ...
            print(
                '\n# build_version=1.0.280, database_name=eric_shn_dp, bucket=s3://eric-yamato-dp, timeout=28800 minutes, job concurrency=3')
