...
def _create_database(database_name, owner):
    ...
    # remove the 'connect' privilege for all users
    # note that the owner can still connect
    sql_command = 'REVOKE ALL ON DATABASE "%s" FROM public' % database_name

    _run_sql(database_name, sql_command)
...
def _run_sql(database_name, sql_command):
    command = 'psql -d %s -c "%s"' % (database_name, _format_sql(sql_command))

    result = run(command)

    if result.failed:
        raise Exception('Could not run sql command "%s"' % sql_command)
