...
from . import commands, postgresql
...
def prune_obsolete_instances(vhost_path):
    ...
    # Delete all instances except the first
    for instance in unused_instances[1:]:
...
def create_virtualenv(virtualenv_path, python_version):
    ...
    run('virtualenv -p %s %s' % (python_version, virtualenv_path))
